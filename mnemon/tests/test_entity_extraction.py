...
def test_wikilink_multiple():
    refs = extract_entities("Connects [[example-service]] and [[agent-memory]].")
    names = _names(refs)
...
def test_mention_simple():
    refs = extract_entities("Deployed @example-service to staging.")
    assert "example-service" in _names(refs)
    assert _typed(refs)["example-service"] == "mention"
