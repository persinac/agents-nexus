...
import { getAgentSourceConfig, resolveAgentId } from '../office/agentSourceConfig.js';
...
export function useExtensionMessages(
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (msg.type === 'layoutLoaded') {
      ...
      } else if (msg.type === 'clearAllAgents') {
        // Bridge reconnected — wipe all agent state; server will re-announce active agents
        const ids = [...os.characters.keys()];
        for (const id of ids) {
          os.removeAgent(id);
        }
        setAgents([]);
        ...
        const agentSource = msg.agentSource as string | undefined;
        const id = resolveAgentId(msg.id as number, agentSource);
        const folderName = msg.folderName as string | undefined;
        const config = agentSource ? getAgentSourceConfig(agentSource) : null;
        const palette = typeof msg.palette === 'number' ? msg.palette : config?.palette;
        setAgents((prev) => (prev.includes(id) ? prev : [...prev, id]));
        setSelectedAgent(id);
        os.addAgent(id, palette, undefined, undefined, undefined, folderName);
        saveAgentSeats(os);
      } else if (msg.type === 'agentClosed') {
        const id = resolveAgentId(msg.id as number, msg.agentSource as string | undefined);
        ...
        const agentSource = msg.agentSource as string | undefined;
        const fromId = resolveAgentId(msg.fromId as number, agentSource);
        const toId = resolveAgentId(msg.toId as number, agentSource);
