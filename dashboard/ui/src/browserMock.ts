 ...
 *
 * Dispatches asset messages, then connects to the tmux bridge WebSocket
 * server for live agent state updates.
 */
export function dispatchMockMessages(): void {
  ...
  // Connect to tmux bridge server for live agent updates
...
function connectToBridge(dispatch: (data: unknown) => void): void {
  ...
  ws.onopen = () => {
    ...
    // Clear stale agent state — server will immediately re-announce all active agents
    dispatch({ type: 'clearAllAgents' });
  };
  ...
  ws.onclose = () => {
    ...
    setTimeout(() => connectToBridge(dispatch), 2000);
  };
