...
// ── Agent sources plugin ──────────────────────────────────────────────────────

const VIRTUAL_AGENT_SOURCES = 'virtual:agent-sources';
const RESOLVED_VIRTUAL_AGENT_SOURCES = '\0' + VIRTUAL_AGENT_SOURCES;

function parseAgentSourcesYaml(content: string): Record<string, { palette?: number }> {
  const result: Record<string, { palette?: number }> = {};
  ...
  for (const raw of content.split('\n')) {
    ...
    if (currentKey) {
      const propMatch = line.match(/^palette:\s*(\d+)$/);
      if (propMatch) result[currentKey].palette = parseInt(propMatch[1], 10);
    }
  }
  return result;
}

function agentSourcesPlugin(): Plugin {
  const yamlPath = path.resolve(__dirname, 'agentSources.yaml');

  function virtualModule(): string {
    const content = fs.existsSync(yamlPath) ? fs.readFileSync(yamlPath, 'utf8') : '';
    return `export default ${JSON.stringify(parseAgentSourcesYaml(content))};`;
  }

  return {
    name: 'agent-sources',
    resolveId(id) {
      if (id === VIRTUAL_AGENT_SOURCES) return RESOLVED_VIRTUAL_AGENT_SOURCES;
    },
    load(id) {
      if (id === RESOLVED_VIRTUAL_AGENT_SOURCES) return virtualModule();
    },
    configureServer(server) {
      server.watcher.add(yamlPath);
      server.watcher.on('change', (file) => {
        if (file !== yamlPath) return;
        const mod = server.moduleGraph.getModuleById(RESOLVED_VIRTUAL_AGENT_SOURCES);
...
export default defineConfig({
  plugins: [react(), browserMockAssetsPlugin(), agentSourcesPlugin()],
  build: {
    outDir: '../dist/webview',
    emptyOutDir: true,
  },
