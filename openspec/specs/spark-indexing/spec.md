# spark-indexing Specification

## Purpose

Defines how Spark (127 Guilty Spark) discovers, classifies, and incrementally indexes installations into LanceDB. Covers per-installation metadata persistence, change detection via local git HEAD timestamps, re-indexing and pruning behavior, run summarization, and the relationship between `spark sync`, `spark reclaim`, and `spark activate` in the nightly pipeline.
