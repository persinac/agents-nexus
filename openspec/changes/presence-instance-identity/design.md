...
v2 bumps `v` to 2 and carries per-instance identity in a NEW `instances:[{name, workspace, pane}]` field while keeping `agents:[names]` unchanged. This is the load-bearing back-compat choice: a v1 bridge does `obj.agents.map(String)`, so putting records in `agents` would give it `"[object Object]"` names — keeping `agents` as bare names and adding `instances` alongside makes the wire safe in BOTH directions (v1 reads names; v2 reads instances; v1-of-v2 and v2-of-v1 both work). `parsePresence` prefers `instances`, falls back to `agents` (a v1 bare string folds in as `{name, workspace:'', pane:''}`). `formatPresence` emits v2 only when the FQDN flag is on, else v1. **Alternative considered:** put records directly in `agents` under `v:2` — rejected, it corrupts every v1 consumer's view of this host (`"[object Object]"`). **Alternative considered:** a second sentinel (`::nexus-presence2::`) — rejected; one sentinel + a version field + an additive field is simpler and the parser already keys on `::nexus-presence::`.
