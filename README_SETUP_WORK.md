...
gateway (the corporate gateway-style proxy with SSO auth) instead of `api.anthropic.com`
...
- A locally-running tunnel/daemon that exposes the corporate gateway on a
  ...
  ```
- The gateway's path prefix (commonly `/anthropic` for the corporate gateway-shaped proxies).
...
- **The path suffix** matches the gateway's expected route. the corporate gateway-style proxies expose `/anthropic/v1/messages`, so set `ANTHROPIC_API_BASE` to the prefix without `/v1/messages` — the proxy appends it.
