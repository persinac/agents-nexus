...
const SPAWN_SUMMARIES_FILE = process.env.SLACK_SPAWN_SUMMARIES_FILE || join(HOME, '.tmux', 'spawn-summaries.json'); // optional repo-desc cache; hand-written desc in the allowlist always overrides. Absent → {} (loadSummaries), so spawn cards fall back to hand-written descriptions.
const SPAWN_MIN_SCORE = parseFloat(process.env.SLACK_SPAWN_MIN_SCORE || '0'); // permissive: the confirm card is the gate
