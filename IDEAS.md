...
| 32 | 3 | **Picker spawns into a bucket land as panes, not tabs** — a bucketed `spawn` now splits the BIGGEST pane in the bucket on its roomier axis, and only opens a tab once no pane would leave both halves above `NEXUS_SPLIT_MIN_COLS`/`NEXUS_SPLIT_MIN_ROWS` (80x20). Splitting the biggest rather than the focused pane is what stops the repeated-halving slivers that made unconditional tiling bad; a fresh bucket also runs its first agent in the workspace ROOT pane instead of stranding an idle shell next to it. | **done** — 4 usable panes/tab at 215x73, then overflow to a tab |
