...
| 32 | 3 | **Picker spawns into a bucket should land as panes, not tabs** — Alex is "not in love with" the current behavior (2026-08-30, deferred at the time). Pre-herdr-0.8 the seam forced `--split down` on any `--workspace` spawn, so `ctrl+a shift+N` agents tiled into the bucket's existing pane; the 0.8 port made `tab create` the default and splits only on an explicit `--split`. Cheap revert: restore `${split:-down}` so the split path fires whenever `ws_id` is set (`substrate.sh` spawn, herdr branch). But unconditional tiling is what the port moved away from — repeated same-direction splits produce unusable slivers, which herdr's own skill doc warns against — so the real fix is likely a width/height threshold (split while the target pane is big enough, else new tab) or a per-bucket policy. | backlog |
