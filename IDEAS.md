...
| 19 | 4 | **Multi-org GitHub token rotation for Spark** — support multiple GitHub PATs in config (keyed by org), so Spark enrichment (descriptions, PRs) works across `persinac`, `example-org`, `example-org` without needing a single god-token. Fine-grained PATs are scoped per-org. | |
