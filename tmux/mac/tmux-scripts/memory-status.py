...
def db_url():
    ...
    if not url:
        return None
    # psycopg/libpq rejects a `search_path` URI query param ("invalid URI query
    # parameter: search_path"). Our queries are schema-qualified (agents.*), so
    # search_path is unnecessary here — strip it. Then ensure sslmode=require.
