...
# ── Registry guarantee ───────────────────────────────────────────────────────
# EVERY claude session gets a registry entry, however it was launched — so the reaper,
# `peers`, and the bus can see + address it (and it never talks on the bus as "unknown").
# The SessionStart hook fires for all sessions, so it's the universal catch-all. Only
# self-register when nothing already has: a launcher (open-claude / conductor) writes a
# better-named entry BEFORE claude starts, so this never clobbers it — it fills the
# human-launched / any-other-launcher gap. Backend-agnostic; runs before the tmux-only
# idle watcher below (which the herdr early-exit skips).
if [ ! -f "$HOME/.tmux/registry/$PANE" ]; then
  _ss_name="${PROJECT_SLUG:-$(basename "$PWD" 2>/dev/null)}"
  [ -n "$_ss_name" ] && "$HOME/.tmux/substrate.sh" register "$PANE" "$_ss_name" "$PWD" 2>/dev/null
fi

# herdr reports agent_status (idle) natively, so the tmux screen-scrape watcher below
# is a tmux-only workaround — skip it in a herdr pane.
[ "${HERDR_ENV:-}" = "1" ] && exit 0
...
for _ in $(seq 1 40); do                                  # up to ~120s (40 x 3s)
  ...
  [ -n "$("$HOME/.tmux/substrate.sh" pane-opt "$PANE" @waiting 2>/dev/null)" ] && exit 0
  ...
  if printf "%s" "$screen" | grep -qE "for shortcuts|to show tasks|for agents"; then
    "$HOME/.tmux/substrate.sh" report-state "$PANE" idle 2>/dev/null
