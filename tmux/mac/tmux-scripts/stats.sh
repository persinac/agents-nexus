#!/usr/bin/env bash
# APM/fleet stats — prints ONE snapshot and exits. Callers loop it for a live panel
# (see plugins/nexus-observe/bin/apm-panel.sh). herdr-aware: active-agent count via the
# substrate seam; log path via NEXUS_TMUX_DIR (relocatable).
NEXUS_TMUX_DIR="${NEXUS_TMUX_DIR:-$HOME/.tmux}"
LOG="$NEXUS_TMUX_DIR/apm.log"
NOW=$(date +%s)
...
# Active agents via the substrate seam (herdr or tmux) — not a hardcoded tmux session.
ACTIVE=$("$NEXUS_TMUX_DIR/substrate.sh" query 2>/dev/null | grep -c '|' || echo 0)
