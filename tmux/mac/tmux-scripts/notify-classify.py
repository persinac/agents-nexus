...
def classify(name, inp):
    ...
    if short in ("bash", "shell"):
        ...
        # _bash_is_denied stays the SINGLE authority for "permissive mode must not
        # approve this" — it is what the regression suite delegates to, so the two can
        # never drift. `denied_by` is derived purely for the decision log and is
        # deliberately NOT a second implementation of the rule: it only names which
        # clause fired, and only ever runs after the authority has already said no.
        # "the gate asked" and "the destructive guard refused" are different events and
        # were indistinguishable in the logs before the decision log existed.
        safe = bool(cmd) and not _bash_is_denied(cmd)
        denied_by = None if safe or not cmd else (
            "_DENY" if _DENY.search(cmd) else
            "_DESTRUCTIVE" if _DESTRUCTIVE.search(cmd) else
            # rm reaching here means _rm_is_scratch_only refused to vouch for a target.
            # Logged as its own tier: it is the clause most likely to need tuning, and
            # folding it into _DENY would hide exactly the signal that tuning needs.
            "_DENY_RM")
