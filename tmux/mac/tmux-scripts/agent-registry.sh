...
case "$COMMAND" in
  peers)
    ...
    for f in "$REGISTRY_DIR"/*; do
      ...
      if [ "${NEXUS_SUBSTRATE:-herdr}" = "herdr" ]; then
        "$HOME/.tmux/substrate.sh" pane-alive "$pane_id" || { rm -f "$f"; continue; }
        slot="$pane_id"   # herdr: the handle IS the address (no window index)
      else
  ...
  whoami)
    pane_id="${TMUX_PANE:-${HERDR_PANE_ID:-}}"
    ...
    if [ -f "$reg_file" ]; then
      ...
      if [ "${NEXUS_SUBSTRATE:-herdr}" = "herdr" ]; then slot="$pane_id"; else slot=$(tmux display-message -t "$pane_id" -p '#{window_index}' 2>/dev/null); fi
  ...
  broadcast)
    ...
    for f in "$REGISTRY_DIR"/*; do
      ...
      [ -n "$EXCLUDE_PANE" ] && [ "$pane_id" = "$EXCLUDE_PANE" ] && continue
      # Deliver through the substrate seam (also applies the literal-paste + settle
      # delay that raw broadcast previously skipped — more robust against TUI coalescing).
      if [ "${NEXUS_SUBSTRATE:-herdr}" = "herdr" ]; then
        "$HOME/.tmux/substrate.sh" pane-alive "$pane_id" || { rm -f "$f"; continue; }
        "$HOME/.tmux/substrate.sh" send "$pane_id" "$MSG"
      else
        slot=$(tmux display-message -t "$pane_id" -p '#{window_index}' 2>/dev/null || true)
        [ -z "$slot" ] && { rm -f "$f"; continue; }
        "$HOME/.tmux/substrate.sh" send "${SESSION}:${slot}" "$MSG"
      fi
      sent=$((sent + 1))
    done
