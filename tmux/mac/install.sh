...
# herdr UX-parity base config (the batteries: ctrl+a prefix, jump-to-agent, splits,
# focus/swap panes, sidebar). Plugins (e.g. the nexus-fleet repo picker) are OPT-IN
# on top — see scripts/herdr-plugin-install.sh.
if [ -d "$SCRIPT_DIR/herdr" ]; then
  mkdir -p "$HOME/.config/herdr"
  _NEXUS_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
  # Deploy the base config as a COPY (not a symlink) so it's per-machine and can carry
  # opt-in plugin keybindings that scripts/herdr-plugin-install.sh appends.
  cp "$SCRIPT_DIR/herdr/config.toml" "$HOME/.config/herdr/config.toml"
  # Re-append keybindings for any bundled plugin already linked, so a base re-run
  # preserves opt-in chords. (Fresh installs have nothing linked -> config stays base.)
  if command -v herdr >/dev/null 2>&1; then
    _linked="$(herdr plugin list --json 2>/dev/null || true)"
    for _pdir in "$_NEXUS_DIR"/plugins/*/; do
      [ -f "$_pdir/herdr-plugin.toml" ] || continue
      _pid="$(sed -n 's/^id[[:space:]]*=[[:space:]]*"\([^"]*\)".*/\1/p' "$_pdir/herdr-plugin.toml" | head -1)"
      case "$_linked" in
        *"\"$_pid\""*) "$_NEXUS_DIR/scripts/herdr-plugin-install.sh" "$(basename "$_pdir")" || true ;;
      esac
    done
  fi
  herdr server reload-config >/dev/null 2>&1 || true
  echo "Installed herdr base config (~/.config/herdr/config.toml) — plugins are opt-in: scripts/herdr-plugin-install.sh <name>"
fi
...
for plist in "$SCRIPT_DIR"/launchd/*.plist; do
  ...
  launchctl load "$HOME/Library/LaunchAgents/$name" || echo "  WARNING: launchctl load failed for $name (non-fatal — that service just won't auto-run)"
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  if ! grep -q "CLAUDE_EFFORT" "$ENV_FILE"; then
    echo "CLAUDE_EFFORT=\"\${CLAUDE_EFFORT:-xhigh}\"" >> "$ENV_FILE"
...
setup_mcp_config() {
  ...
  local search_paths=(
    "$HOME/minions/example-repo/agent-memory"
    "$HOME/repos/example-repo/agent-memory"
    "$HOME/repos/example-repo/agent-memory"
    "$HOME/projects/example-repo/agent-memory"
  )
