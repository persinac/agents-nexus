...
function tmuxListWindows() {
  try {
    ...
    const out = execSync(
      substrate + " query",
      { encoding: "utf8", timeout: 2000, env: { ...process.env, TMUX_AGENT_SESSION: TMUX_SESSION } }
    ).trim();
    return out.split("\n").filter(Boolean).map(line => {
      const [index, name, waiting, path, command, waitType] = line.split("|");
      // Keep the id opaque: numeric window-index in tmux mode, string pane handle
      // (w1:p1) in herdr mode. parseInt on a herdr handle → NaN → all agents collide.
      return { index: /^\d+$/.test(index) ? parseInt(index, 10) : index, name, waiting, path, command, waitType: waitType || "" };
    });
  } catch {
    return [];
  }
...
const httpServer = http.createServer((req, res) => {
  ...
  if (respondMatch && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        // Match the poll() key type: numeric in tmux mode, string handle in herdr mode.
        const agentId = /^\d+$/.test(respondMatch[1]) ? parseInt(respondMatch[1], 10) : respondMatch[1];
        const agent = agents.get(agentId);
        if (!agent) {
        ...
        // Route keystroke responses through the substrate seam (tmux today; herdr under
        // NEXUS_SUBSTRATE=herdr). send-keys maps enter/escape to named keys and a bare
        // char (y) to a literal keystroke.
        const substrate = `"${process.env.HOME}/.tmux/substrate.sh"`;
        // herdr addresses panes by bare handle (w1:p1); tmux by session:window-index.
        const dest = process.env.NEXUS_SUBSTRATE !== 'tmux' ? String(agentId) : `${TMUX_SESSION}:${agentId}`;  // default herdr (handle); tmux (session:idx) only when explicit
        if (action === 'approve') {
          if (wType === 'elicitation_dialog') {
            // AskUserQuestion / edit acceptance — confirm with Enter
            execSync(`${substrate} send-keys "${dest}" enter`, { timeout: 3000 });
          } else {
            // Permission prompt — approve with 'y'
            execSync(`${substrate} send-keys "${dest}" y`, { timeout: 3000 });
          }
        } else if (action === 'deny') {
          execSync(`${substrate} send-keys "${dest}" escape`, { timeout: 3000 });
        } else {
  ...
  if (url.pathname === '/api/system/health') {
    ...
    try {
      ...
      let tmux = false;
      try { execSync(process.env.HOME + "/.tmux/substrate.sh has-session", { timeout: 3000, env: { ...process.env, TMUX_AGENT_SESSION: TMUX_SESSION } }); tmux = true; } catch {}
