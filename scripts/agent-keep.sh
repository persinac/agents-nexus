...
# Precondition routed through the substrate seam. In herdr mode there is no tmux
# `agents` session, so the old `tmux has-session` guard exited HERE — before the
# `substrate keep` write ran — silently failing the pin. The reaper then read no
# @keep and could sweep an agent the user believed was protected. has-session is
# backend-aware (herdr: `herdr status server`; tmux: `tmux has-session`).
"$SUBSTRATE" has-session 2>/dev/null || { echo "agent-keep: fleet substrate ($BACKEND) is not up" >&2; exit 1; }
...
list_kept() {
  ...
  if [ "$BACKEND" = herdr ]; then
    # herdr has no `tmux list-windows`; enumerate the registry (the reaper's own
    # source of truth) and read @keep per pane through the seam (daemon/sidecar).
    local name pane
    for f in "$REGISTRY_DIR"/*; do
      [ -f "$f" ] || continue
      name=""; pane=""
      ...
      [ -n "$pane" ] || continue
      [ "$("$SUBSTRATE" pane-opt "$pane" @keep 2>/dev/null)" = "1" ] || continue
      printf '  @keep  %-24s %s\n' "${name:-?}" "$pane"; any=1
    done
...
case "$TARGET" in
  ...
  w[A-Za-z0-9]*:p[A-Za-z0-9]*)  TGT="$TARGET" ;;       # herdr pane handle (wN:pN) — pass through
...
if ! "$HOME/.tmux/substrate.sh" keep "$TGT" "$VAL" 2>/dev/null; then
